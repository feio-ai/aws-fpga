...
namespace eval debug_profile {
  ...
  proc add_debug_profile_support {hw_platform_info config_info debug_profile_info} {
    ...
    fill_global_information $hw_platform_info $config_info $debug_profile_info
    ...
    set filename "${output_dir}/insert_debug_profiling.tcl"
    set fp [open $filename w]
    
    # Until we unify the emulation and hardware flows, 
    #  we will treat them completely separately
    if {$is_hw_emu} {
      # Debug not supported in HW emu
      #insert_debug_hwemu   $hw_platform_info $config_info $debug_profile_info
      insert_profile_hwemu $hw_platform_info $config_info $debug_profile_info $fp
    } else {
      # Insert all debug/profile monitors
      add_debug_profile $fp $config_info $debug_profile_info
      # Insert ILAs
    ...
    puts $fp "debug_profile::write_debug_ip_layout $is_hw_emu \"$hw_platform_vbnv\" \"$output_dir\""
    close $fp
    return $filename
  ...
  ##############################################################
  proc write_debug_ip_layout { {is_hw_emu false} {hw_platform_vbnv "none"} {output_dir "."} } {
    ...
    if {$num_entries > 0} {
      foreach debug_ip $debug_ip_list {
        ...
        #Special handeling for AXI_TRACE_FUNNEL
        if {$is_hw_emu && ($type == "AXI_TRACE_FUNNEL")} {
          ...
          for { set a 0 } {$a < $amax } { incr a } {
            ...
            # Not finding address is still valid for HW emulation, so just report as INFO
            if {$base_address == 0xdeadbeef} {
              puts "INFO: Unable to find address for IP $debug_ip"
            }
        ...
        if {($type == "TRACE_S2MM") && ([string first "aie" $debug_ip] >= 0)} {
          ...
          if {($traceAddress != {}) && ($traceDepth != {})} {
            # Check if ap_start exists and is tied high
            set startProp 0
            set startPin [get_bd_pins -quiet "${debug_ip}/ap_start"]
            if {$startPin != {}} {
              set startDriverPin  [find_bd_objs -quiet -thru_hier -relation connected_to $startPin]
              set startDriverCell [get_bd_cells -quiet -of $startDriverPin]
              if {[get_property -quiet CONFIG.CONST_VAL $startDriverCell] == 1} {
                set startProp 1
              }
            }

            #puts "--- DEBUG IP: trace address = $traceAddress, depth = $traceDepth"
            write_debug_ip_entry $debug_ip $fp "TRACE_S2MM_FULL" 0 $startProp $traceAddress $traceDepth false
            incr i
          }
        }
