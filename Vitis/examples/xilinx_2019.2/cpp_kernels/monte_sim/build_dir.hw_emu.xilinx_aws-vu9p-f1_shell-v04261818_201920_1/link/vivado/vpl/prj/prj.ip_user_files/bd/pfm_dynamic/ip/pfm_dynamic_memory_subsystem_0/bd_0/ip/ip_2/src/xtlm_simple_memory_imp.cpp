...
class xtlm_simple_memory_imp::target_rd_util: public xtlm::xtlm_aximm_target_rd_socket_util {
public:
	target_rd_util(sc_core::sc_module_name p_name,
			xtlm::aximm::granularity g_hint, int width_p,
			xtlm_simple_memory_imp* _parent) :
...
private:
	xtlm_simple_memory_imp* parent;
};

class xtlm_simple_memory_imp::target_wr_util: public xtlm::xtlm_aximm_target_wr_socket_util {
public:
...
xtlm_simple_memory_imp::xtlm_simple_memory_imp(
		const sc_module_name& module_name,
		xsc::common_cpp::properties& properties) :
		sc_module(module_name) {
	m_data_width = properties.getLongLong("C_S00_AXI_DATA_WIDTH");
	m_mem_addr_width = properties.getLongLong("C_MEM_SIZE");
	...
	if (!getenv("SDX_USE_SHARED_MEMORY"))
		m_mem_handler = new xtlm_simple_axi_ram_fmodel(this->name(),
				m_report_handler, m_mem_addr_width); //name() returns hierarchical name of ddr systemc module instance
	else
		m_mem_handler = new xtlm_simple_fmodel_shared_memory(this->name(),
				m_report_handler, m_mem_addr_width); //name() returns hierarchical name of ddr systemc module instance

	//m_mem_handler=new xtlm_simple_memory_fmodel_base(mod_name,m_report_handler,m_mem_addr_width);
...
void xtlm_simple_memory_imp::methodProcessTransactionRead() {
	std::stringstream ss;
...
void xtlm_simple_memory_imp::methodProcessTransactionWrite() {
	std::stringstream ss;
			...
			if (ext != nullptr) {
				...
				if (ext->getType() == xsc::extension::TYPE::ALLOC_BO)
					ret = m_mem_handler->createMMappedBuffer(
							trans->get_address(), ext->getSize(), filename);
				else if (ext->getType() == xsc::extension::TYPE::FREE_BO)
					ret = m_mem_handler->freePage(trans->get_address());
				else if (ext->getType() == xsc::extension::TYPE::IMPORT_BO) {
					ret = m_mem_handler->importBO(trans->get_address(),
							ext->getFileName(), ext->getSize());
				} else if (ext->getType() == xsc::extension::TYPE::COPY_BO) {
					ret = m_mem_handler->copyBO(trans->get_address(),
							ext->getFileName(), ext->getSize(),
...
unsigned int xtlm_simple_memory_imp::transport_dbg(xtlm::aximm_payload& trans) {
...
unsigned int xtlm_simple_memory_imp::transport_dbg_rd(
		xtlm::aximm_payload& trans) {
	sc_core::sc_time delay = SC_ZERO_TIME;
	uint64_t max_address = m_mem_addr_width;
	uint64_t address = trans.get_address() % max_address;
...
unsigned int xtlm_simple_memory_imp::transport_dbg_wr(
		xtlm::aximm_payload& trans) {
	std::stringstream ss;
	uint64_t max_address = m_mem_addr_width;
	uint64_t address = trans.get_address() % max_address;
	...
	if (ext != nullptr) {
		...
		bool ret = true;
		if (ext->getType() == xsc::extension::TYPE::ALLOC_BO)
			ret = m_mem_handler->createMMappedBuffer(trans.get_address(),
			...
			ret = m_mem_handler->copyBO(trans.get_address(), ext->getFileName(),
