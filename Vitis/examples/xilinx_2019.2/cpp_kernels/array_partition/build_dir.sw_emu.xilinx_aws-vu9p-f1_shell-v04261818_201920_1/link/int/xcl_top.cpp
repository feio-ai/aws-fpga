...
void matmul_partition(size_t in1, size_t in2, size_t out_r, int size);
...
void __stub____xlnx_cl_matmul_partition(char **argv) {
 ...
 pthread_mutex_lock(&__xlnx_cl_matmul_partition_mutex);
  matmul_partition(in1, in2, out_r, size);
  pthread_mutex_unlock(&__xlnx_cl_matmul_partition_mutex);
}
void matmul(size_t in1, size_t in2, size_t out_r, int size);

static pthread_mutex_t __xlnx_cl_matmul_mutex = PTHREAD_MUTEX_INITIALIZER;
void __stub____xlnx_cl_matmul(char **argv) {
  void **args = (void **)argv;
  size_t in1 = *((size_t*)args[0+1]);
  size_t in2 = *((size_t*)args[1+1]);
  size_t out_r = *((size_t*)args[2+1]);
  int size = *((int*)args[3+1]);
 pthread_mutex_lock(&__xlnx_cl_matmul_mutex);
  matmul(in1, in2, out_r, size);
  pthread_mutex_unlock(&__xlnx_cl_matmul_mutex);
}
}
