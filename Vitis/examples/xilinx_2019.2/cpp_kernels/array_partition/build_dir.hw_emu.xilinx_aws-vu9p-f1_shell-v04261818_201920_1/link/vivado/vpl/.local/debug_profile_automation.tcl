...
namespace eval dpa {
  ...
  proc get_dpa_axilite_master { axiliteDict } {
    ...
    #   3. GP0 on CIPS (add if not already present)
    set axiliteMaster  [get_bd_intf_pins -quiet [dict_get_default $axiliteDict MASTER ""]]
    if {$axiliteMaster == ""} {
      set axiliteMaster [get_bd_intf_pins -quiet -hier -filter {HDL_ATTRIBUTE.DPA_AXILITE_MASTER == true}]
    }
    ...
    set masterPins [get_bd_intf_pins -quiet -of_objects $cips -filter {NAME=~"*GP*" && CONFIG.PROTOCOL == AXI4 && MODE == Master}]
    if {$masterPins != ""} {
      # Add SmartConnect to first GP port
      set newMaster [add_axilite_smartconnect [lindex $masterPins 0]]
      return $newMaster
    }
      
    send_msg_id "DPA-1" "WARNING" "Adding GP0 master to $cips. Clock must be connected manually."
    set_property CONFIG.PS_USE_M_AXI_GP0 1 $cips
    return [get_bd_intf_pins $cips/M_AXI_GP0]
    ...
    if {[get_bd_cells -quiet $traceOffloadSlave] != {}} {
      ...
      if {[string first "memory_subsystem" $vlnv] >= 0} {
        ...
        if {[string first "axi_noc" $vlnv] >= 0} {
          ...
          # Add connections and set each QoS to 100 MB/s
          # TODO: What is the correct QoS?
          set connections {}
          set bwDict { read_bw {0} write_bw {100} }
        ...
        foreach type $typelist {
          ...
          if {$type == "data"} {
            ...
            if {[is_stream $obj]} {
              ...
              add_axi_stream_protocol_checker $obj $temp $trace_clk $trace_rst $monName  
            } else {
              # Add Lightweight AXI Protocol Checker (LAPC)
                add_axi_protocol_checker $obj $temp $trace_clk $trace_rst $monName
            }
          } else {
  ...
  proc add_accel_monitor {cu adict trace_clk trace_rst monName dpa_opts} {
    ...
    # Only enable stall monitoring if 1. stall monitoring was requested; and 2. the CU was compiled with stalls
    ...
    if { $ctrlInterface != {} } { 
      ...
      set_property CONFIG.MONITOR_MODE 0 $monObject
  ...
  proc add_axi_stream_monitor {intf_pin adict trace_clk trace_rst monName dpa_opts} {
    ...
    set mon_clk [get_bd_pins -quiet [dict_get_default $adict CLK_SRC ""]]
    set mon_rst [get_bd_pins -quiet [dict_get_default $adict RST_SRC ""]]
    set mon_id  [dict_get_default $adict MON_ID ""]
    ...
    set_property CONFIG.EN_AXI_LITE  1 $monObject   
    if {$is_hw_emu} {
      set_property CONFIG.MONITOR_ID $mon_id $monObject
    }
    ...
    set pinEnd  [find_bd_objs -relation connected_to $pinObj]
  ...
  proc add_axi_interface_monitor {intf_pin adict trace_clk trace_rst baseMonName dpa_opts} {
    ...
    if {$is_hw_emu} {
      set_property CONFIG.MONITOR_ID [lindex $mon_id $i] $monObject
    }
  ...
  #    None
  ###########################################################
  proc add_axi_stream_protocol_checker {intf_pin adict trace_clk trace_rst monName} {
  ...
  #    None
  ###########################################################
  proc add_axi_protocol_checker {intf_pin adict trace_clk trace_rst monName} {
  ...
  # get_axilite_clock
  #  Description:
  #    Find the clock signal used by the AXI-lite control 
  #  Arguments:
  #    axiliteDict   A dictionary of all AXI-Lite information
  #    axiliteMaster  The AXI-Lite master port
  #  Return Value:
  #    The bd_pins or {}
  ###########################################################
  proc get_axilite_clock { axiliteDict axiliteMaster } {
    set axiliteClock [get_bd_pins -quiet [dict_get_default $axiliteDict  CLK_SRC ""]]
    if {$axiliteClock == {}} {
      set axiliteClock   [bd::clkrst::get_sink_clk $axiliteMaster]
    }
    return $axiliteClock
  ...
  # get_axilite_reset
  #  Description:
  #    Find the reset signal used by the AXI-lite control 
  #  Arguments:
  #    axiliteDict   A dictionary of all AXI-Lite information
  #    axiliteClock  The AXI-Lite control clock
  #  Return Value:
  #    The bd_pins or {}
  ###########################################################
  proc get_axilite_reset { axiliteDict axiliteClock } {
    # First try the dictionary
    set axiliteReset [get_bd_pins -quiet [dict_get_default $axiliteDict RST_SRC ""]]
    if {$axiliteReset == {}} {
      set axiliteReset   [bd::clkrst::get_src_rst  $axiliteClock]
      
      if {$axiliteReset == {}} {
        set axiliteReset [get_bd_pins -quiet -hier -filter {HDL_ATTRIBUTE.DPA_AIE_TRACE_RESET == true}]
      }
    }
    return $axiliteReset
  }
  ...
  proc add_trace_infrastructure {dpa_opts} {
    ...
    set isDedicated              [dict_get_default $offloadDict  DEDICATED     true]
    ...
    if {$traceMemory == "FIFO"} {
      set traceOffloadMaster     [get_bd_intf_pins -quiet [dict_get_default $offloadDict MASTER ""]]
      ...
      if {$traceOffloadReset == "" && $is_embedded == "true"} {
        ...
        set axiliteMaster    [get_dpa_axilite_master $axiliteDict]
        if {$axiliteMaster == {}} {
          send_msg_id "DPA-16" "CRITICAL_WARNING" "Unable to insert AXI-Lite control. No master found."
          return
        }
    
        set axiliteClock [get_axilite_clock $axiliteDict $axiliteMaster]
        set traceOffloadReset [get_axilite_reset $axiliteDict $axiliteClock]
      }
    ...
    if {$traceMemory == "FIFO"} {
      ...
      if { !$isDedicated } {
        ...
        if {$maxOffset > 0} {
          ...
          if {$fifoSlaveSeg != {}} {
            putd "--- DPA: Setting address offset to [format 0x%X $maxOffset] for $fifoSlaveSeg"
            puts "------ DEBUG: Setting address offset to [format 0x%X $maxOffset] for $fifoSlaveSeg"
      ...
      # Instantiate trace S2MM or AXI DMA
      set USE_TRACE_S2MM 1
      if {$USE_TRACE_S2MM} {
        # Use trace S2MM HLS kernel
        ...
        # Use AXI DMA
        set dmaName "dpa_trace_dma"
        set dmaObject [create_bd_cell -type ip -vlnv xilinx.com:ip:axi_dma $dmaName]
        ...
        connect_bd_net $traceClock [get_bd_pins "${dmaName}/s_axi_lite_aclk"]
        connect_bd_net $traceClock [get_bd_pins "${dmaName}/m_axi_s2mm_aclk"]
        connect_bd_net $traceReset [get_bd_pins "${dmaName}/axi_resetn"]
      
        set dmaStreamName "S_AXIS_S2MM"
        set dmaSlaveName "S_AXI_LITE"
        set dmaMasterName "M_AXI_S2MM"
      }
  ...
  proc add_aie_trace_infrastructure {dpa_opts} {
    ...
    # Connect each stream to the interconnect
    for { set s 0 } { $s < $numStreams } { incr s } {
      set currStream [lindex $aieStreams $s]
      putd "--- DPA: Connecting AIE trace stream $currStream..."
      
      # Make sure this stream doesn't already have a slave
      remove_all_slaves $currStream
      
      set fifoPropName [expr { ($s >= 10) ? "CONFIG.S${s}_FIFO_DEPTH" : "CONFIG.S0${s}_FIFO_DEPTH" } ]
      set_property $fifoPropName $fifoDepth $interconObj
      
      set slaveName [expr { ($s >= 10) ? "$interconName/S${s}_AXIS" : "$interconName/S0${s}_AXIS" } ]
      connect_bd_intf_net $currStream [get_bd_intf_pins $slaveName]
      
      set currClock  [bd::clkrst::get_sink_clk $currStream]
      set slaveClockName [expr { ($s >= 10) ? "$interconName/S${s}_AXIS_ACLK" : "$interconName/S0${s}_AXIS_ACLK" } ]
      if {$currClock != {}} {
        connect_bd_net $currClock [get_bd_pins $slaveClockName]
      } elseif {$masterClock != {}} {
        connect_bd_net $masterClock [get_bd_pins $slaveClockName]
      }
      
      set currReset  [bd::clkrst::get_sink_rst $currClock]
      set slaveResetName [expr { ($s >= 10) ? "$interconName/S${s}_AXIS_ARESETN" : "$interconName/S0${s}_AXIS_ARESETN" } ]
      if {$currReset != {}} {
        connect_bd_net $currReset [get_bd_pins $slaveResetName]
      } elseif {$masterReset != {}} {
    ...
    if {$USE_TRACE_S2MM} {
      # Use trace S2MM HLS kernel
      set dmaName "dpa_aie_trace_s2mm"
      set dmaObject [create_bd_cell -type ip -vlnv $trace_s2mm_name $dmaName]
      ...
      connect_bd_net $masterClock [get_bd_pins "${dmaName}/s_axi_lite_aclk"]
      connect_bd_net $masterClock [get_bd_pins "${dmaName}/m_axi_s2mm_aclk"]
      if {$masterReset != {}} {
        connect_bd_net $masterReset [get_bd_pins "${dmaName}/axi_resetn"]
      }
    ...
    connect_bd_intf_net [get_bd_intf_pins "${interconName}/M00_AXIS"] [get_bd_intf_pins "${dmaName}/$dmaStreamName"]
    ...
    set_property HDL_ATTRIBUTE.DPA_IP            true     $dmaObject
    set_property HDL_ATTRIBUTE.DPA_IP_PROPERTIES 1        $dmaObject
    set_property HDL_ATTRIBUTE.DPA_IP_FULLNAME   $dmaName $dmaObject
    
    # Find and connect to trace slave
    set offloadDict        [dict_get_default $dpa_opts     TRACE_OFFLOAD {}]
    ...
    # Connect to trace slave
    connect_trace_offload_slave $traceOffloadSlave $traceMemoryType $traceMemory ${dmaName}/${dmaMasterName} $masterClock $masterReset
    
    # Specify base address and depth (only if specified)
    if { [dict exists $traceDict BASE_ADDRESS] } {
      set baseAddress [dict_get_default $traceDict BASE_ADDRESS 0x0]
      set depth       [dict_get_default $traceDict DEPTH        134217728]
      send_msg_id "DPA-15" "INFO" "Setting base address to $baseAddress and depth to $depth for trace S2MM core."
      
      set slave [find_bd_objs -quiet -relation connected_to -thru_hier [get_bd_intf_pins ${dmaName}/${dmaMasterName}]]
      ...
      putd "--- DPA: Setting address for segment $seg: offset = $baseAddress, range = $depth"
      set_property RANGE  $depth       $seg
      set_property OFFSET $baseAddress $seg
      
      if {$USE_TRACE_S2MM} {
        # Expose raw AP control signals and specify 64-bit max word count
        set_property CONFIG.USE_AP_CTRL_HS     1 $dmaObject
        set_property CONFIG.USE_BUF_WORD_COUNT 1 $dmaObject
        set_property CONFIG.BUF_WORD_COUNT     [expr $depth / 64] $dmaObject
 
        # Connect AP start to '1' on trace S2MM       
        set constName "dpa_aie_trace_const"
        ...
        connect_bd_net [get_bd_pins $constName/dout] [get_bd_pins $dmaName/ap_start]
      }
    }
